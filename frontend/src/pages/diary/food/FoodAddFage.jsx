...
const FoodAddFage = () => {
  ...
  const postFood = async (selectedFoodList) => {
    for (const foodId of selectedFoodList) {
      try {
        await axios.post(`${baseUrl}api/management/food/list/add`, {
      ...
      <div className="fixed w-full bg-white bottom-16">
        <div className="m-4">
          <div className="flex gap-4">
            <Button
              btnCss={
                "basis-1/2 text-center border border-teal-700 rounded-md p-2"
              }
              buttonName={"직접 입력하기"}
            ...
            <Button
              btnCss={
                "basis-1/2 text-center text-white bg-teal-700 rounded-md p-2 "
              }
              buttonName={"추가하기"}
